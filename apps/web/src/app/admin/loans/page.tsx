...
export default function AdminLoansPage() {
  ...
  const { useLoans } = useAdmin();
  const { data, isLoading, error, changePage, changeLimit, search, params } = useLoans();
        ...
        <div className="bg-white p-6 rounded-lg shadow-md mb-6">
          <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
            <div className="flex-grow">
              <input
                type="text"
                placeholder="Search loans by user email, purpose, or ID"
                    ...
                    {data?.loans?.map((loan) => (
                      <tr key={loan.id}>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{loan.user_email}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900">{loan.purpose}</div>
                          <div className="text-xs text-gray-500">{loan.duration} months</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          ${loan.amount?.toFixed(2)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${loan.status === 'approved' ? 'bg-green-100 text-green-800' : loan.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : loan.status === 'rejected' ? 'bg-red-100 text-red-800' : loan.status === 'paid' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
                            {loan.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex space-x-2">
                            <a href={`/admin/loans/${loan.id}`} className="text-[#FF7B00] hover:text-orange-600">
                              View
                            </a>
                            {loan.status === 'pending' && (
                              <>
                                <button 
                                  onClick={() => window.location.href = `/admin/loans/${loan.id}/approve`}
                                  className="text-green-600 hover:text-green-800"
                                >
                                  Approve
                                </button>
                                <button 
                                  onClick={() => window.location.href = `/admin/loans/${loan.id}/reject`}
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing <span className="font-medium">{data?.loans?.length ? (params.page - 1) * params.limit + 1 : 0}</span> to{' '}
                      <span className="font-medium">
                        {data?.loans?.length ? Math.min(params.page * params.limit, data.total) : 0}
                      </span>{' '}
