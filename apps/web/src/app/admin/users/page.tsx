...
export default function AdminUsersPage() {
  ...
  const { data, isLoading, error, changePage, changeLimit, search, params } = useUsers();
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {data?.users?.map((user) => (
                      <tr key={user.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${user.kyc_status === 'approved' ? 'bg-green-100 text-green-800' : user.kyc_status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800'}`}>
                            {user.kyc_status}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <a href={`/admin/users/${user.id}`} className="text-[#FF7B00] hover:text-orange-600">
                            View Details
                          </a>
                        </td>
                      </tr>
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing <span className="font-medium">{data?.users?.length ? (params.page - 1) * params.limit + 1 : 0}</span> to{' '}
                      <span className="font-medium">
                        {data?.users?.length ? Math.min(params.page * params.limit, data.total) : 0}
                      </span>{' '}
