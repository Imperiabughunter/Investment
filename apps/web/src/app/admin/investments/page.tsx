...
export default function AdminInvestmentsPage() {
  ...
  const { useInvestments } = useAdmin();
  const { data, isLoading, error, changePage, changeLimit, search, params } = useInvestments();
        ...
        <div className="bg-white p-6 rounded-lg shadow-md mb-6">
          <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
            <div className="flex-grow">
              <input
                type="text"
                placeholder="Search investments by user email, plan, or ID"
                    ...
                    {data?.investments?.map((investment) => (
                      <tr key={investment.id}>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{investment.user_email}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900">{investment.plan_name}</div>
                          <div className="text-xs text-gray-500">{investment.duration} months</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          ${investment.amount?.toFixed(2)}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${investment.status === 'active' ? 'bg-green-100 text-green-800' : investment.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : investment.status === 'completed' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
                            {investment.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <a href={`/admin/investments/${investment.id}`} className="text-[#FF7B00] hover:text-orange-600">
                            View Details
                          </a>
                        </td>
                      </tr>
                ...
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm text-gray-700">
                      Showing <span className="font-medium">{data?.investments?.length ? (params.page - 1) * params.limit + 1 : 0}</span> to{' '}
                      <span className="font-medium">
                        {data?.investments?.length ? Math.min(params.page * params.limit, data.total) : 0}
                      </span>{' '}
